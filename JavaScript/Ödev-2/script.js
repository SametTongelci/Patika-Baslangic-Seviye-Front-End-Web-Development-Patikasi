...
//Variables
const arrayTaskList = localStorage.getItem("Tasks")
...
function newElement() {
  if (TASK_INPUT.value == null || TASK_INPUT.value.trim() == "") {
    ...
    error.classList.add(`bg-danger`, `text-light`); //! this is an extra code.
    ...
    success.classList.add(`bg-success`, `text-light`); //! this is an extra code.
...
function deleteElement(event) {
  ...
  let arrayIndexOfNodeValue = arrayTaskList.indexOf(
    node.parentNode.childNodes[0].nodeValue
  );
